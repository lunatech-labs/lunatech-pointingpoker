...
supersedes the phases below with a nine-step ordered path, and each item here
notes the step that absorbs it. Steps 0 to 6 close every documented defect; 7 is
a usability improvement and 8 and 9 are product work. Items that stay in a phase
are product decisions built on that target rather than steps toward it.
...
- [ ] Slug-based room ids replacing raw UUIDs, auto-generated adjective-noun
      triples, unique among the rooms currently in memory. **Becomes step 7.**
...
- [ ] A facilitator-recorded round outcome: teams often resolve a split by
      ...
      **step 9** pairs it with the item below.
- [ ] Round history within a session (running list of context to final estimate
      ...
      you cannot look back at is half a feature.

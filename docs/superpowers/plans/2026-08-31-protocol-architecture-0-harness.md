...
4. **`sbt Universal/stage` is required before `npm test`, not only before `npm run e2e`.** 08-30 §Delivery's follow-up note says otherwise, but `test/reproduction.test.js` runs the real app. The README says the accurate thing. Superseded by step 1's deviation 1, which stages from the npm scripts.

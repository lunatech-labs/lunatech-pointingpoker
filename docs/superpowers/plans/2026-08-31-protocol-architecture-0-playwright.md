...
| A departure is announced while another participant is cut | `room.spec.js` | green | deviation 4 below, the control for the pruning case specifically |
...
test/startup.test.js       # new: 1 case, the bind failure of deviation 10
e2e/fixtures.js            # new: app, stub, room, join fixtures + shared locators
e2e/smoke.spec.js          # new: 1 case
e2e/room.spec.js           # new: 11 cases
src/.../Main.scala         # deviation 10, the only src change beyond the pin
src/.../pages/index.html   # deviation 11, the axios pin
.github/workflows/ci.yml   # + npm cache and npm ci, + 5 browser steps
...
- Carol's leave is broadcast and Alice observes it. That same broadcast is a write to Bob's cut stream, and it fails outright rather than only drawing a reset, so Bob's own `ConfirmLeave` is scheduled for about 1.1s + G after it, and that, not the 15s heartbeat, is the budget his reconnect has. The difference from Carol, who needed two writes, is elapsed time and not the kind of teardown: `cut` and a closed tab both reach the app through the same `up.destroy()` in the stub's forwarder, but Carol's two broadcasts land within milliseconds of her disconnect, before the reset has propagated, while Bob's first post-cut write is seconds later against a connection already known to be dead. **One term this derivation leaves out: the heartbeat can get there first.** It only holds while everything from Bob's join to his restore stays inside the hardcoded 15s (`SSE.scala:34`), and three Firefox joins plus the two nudges plus Carol's roughly 5.1s can approach that on a loaded two-core runner running two workers. A heartbeat landing inside the cut window is a write like any other, so it starts Bob's removal clock early and his margin shrinks by however much earlier it lands. That is not a reason to change anything today, since the green control turns the loss into a loud failure rather than a silent one, and raising `SSE_GRACE_PERIOD` is still the right lever. It is a reason not to read the 1.1s + G budget as the whole story when that lever next gets pulled.
...
- `npm run e2e`: 24 passed, 10 of them expected failures, no flakes across two consecutive runs. Twelve cases in each project, which is every row of the table above.
- `git status --short`: empty. No `node_modules/`, `test-results/` or `playwright-report/` tracked, and `package-lock.json` committed.
- `git diff main...HEAD --stat`: nothing under `src/` except `Main.scala` and `pages/index.html`, the two files deviations 10 and 11 declare. `Main.scala` carries three edits within it, all three declared by deviation 10.
...
The task blocks above are the record of what was instructed, and they are left as written. Nine entries arrived that no task block introduces, each in response to something the work turned up: the first four during implementation, the last five from the whole-branch reviews that followed. They are listed here rather than under "Deviations" because that list is for departures from the specs, not from this plan.
...
- **`join()` asserts the page mounted before it fills the name**, bounded at 15s, and `playwright.config.js` sets `navigationTimeout: 20_000`. The mount assertion landed first, on the theory that a *stalling* CDN surfaced as `fill()` waiting out the 60s test timeout; a later review showed it could not, and the config line is the correction. `page.goto` defaults to `waitUntil: 'load'`, and the page's three script tags are classic parser-blocking scripts under a stylesheet that blocks them in turn, so a stall hangs inside the navigation and never reaches the assertion. `waitUntil: 'domcontentloaded'` would not have helped either, since parser-blocking scripts delay `DOMContentLoaded` too. Only a navigation bound catches it, and unlike the mount assertion it also covers `e2e/smoke.spec.js`. The two now split the job: `navigationTimeout` owns the transport, the 15s assertion owns the mount, which is real because the name input renders under `v-if` and needs Vue. An unreachable CDN was never the slow case: the scripts fail at once, `v-if` never evaluates, both name-input blocks render, and `fill()` fails on a strict-mode violation in milliseconds. Both bounds narrow what passes, since `retries` is 0, but a cold first load is four concurrent 5s-bounded asset fetches and the `Show votes` assertion on the default 5s expect timeout is tighter anyway.
- **The asset cache memoizes failures, not only successes** (`e2e/fixtures.js`). Both failure paths used to fall through to `route.continue()` without recording anything, so a dead or stalling CDN was paid its full 5s bound on every one of roughly 55 contexts rather than once per worker, which is the guarantee the fixture exists to make. Caching `null` and falling back on it costs one worker its cache for that URL after a single transient failure, which is exactly the pre-cache behaviour and the right trade against blowing the test timeout. This is the other half of the entry above: the fallback hands the request to the browser, so without a navigation bound the memoization only moves the stall rather than ending it.
- **Six smaller changes from a later whole-branch review**, two of them touching what a case asserts. `departureWhileCut` now asserts the three-participant precondition on Bob's page as well as Alice's: Bob's is the one the case is about, and Alice's guards her own removal assertion from passing on a participant she never had. The 20s cap on the leave case goes to 25s, because a detection that fell back to the 15s heartbeat lands at about 20.1s, just outside it; the identical 20s inside `departureWhileCut` is deliberately left alone, since that wait runs while Bob is cut and a longer one would spend the reconnect budget the case exists to prove rather than buy anything. The other four are hygiene: `playwright.config.js` gains `forbidOnly` on CI, because a stray `test.only` would narrow a characterization suite to one case and still exit 0; the `room` fixture releases the response body before throwing on a non-ok `/create-room`, which is the leak class two earlier fixes already closed elsewhere; `join()` tracks a closer the moment the context exists rather than after `route()` and `newPage()`, so a context cannot outlive a throw between them; and `testkit/app.js` caps its captured output at a 1MiB tail, since the app logs every room broadcast at DEBUG and the attachment is only ever read from the end. The cap took a follow-up commit: dropping chunks until the total fits would discard a single chunk larger than the cap outright, leaving `output()` empty and a startup failure with nothing to explain it, so it now keeps the last chunk whatever its size.
- **The harness spawns the app with `HOST=127.0.0.1` and advertises that address**, rather than `HOST=localhost` with a `http://localhost:<port>` base URL. Deviation 6 had already made the stub advertise the address it bound; the app kept a hostname whose two resolutions disagree, node preferring `::1` and the JVM `127.0.0.1`, so every probe and every `app.baseUrl` fetch worked on undici's fallback after a failed connect. Binding the literal address makes the family explicit on both sides, which is also what removes the resolution-order assumption `test/startup.test.js` used to rest on.

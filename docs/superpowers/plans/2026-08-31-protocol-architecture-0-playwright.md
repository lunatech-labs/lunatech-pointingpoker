...
The task blocks above are the record of what was instructed, and they are left as written. Eight entries arrived that no task block introduces, each in response to something the work turned up: the first four during implementation, the last four from the whole-branch reviews that followed. They are listed here rather than under "Deviations" because that list is for departures from the specs, not from this plan.

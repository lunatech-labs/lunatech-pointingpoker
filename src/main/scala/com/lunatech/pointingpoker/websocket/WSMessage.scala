...
object WSMessage {
  ...
  object MessageType {
    def apply(messageType: String): MessageType = messageType match {
    ...
    def unapply(messageType: MessageType): Option[String] = messageType match {
    ...
    implicit val messageTypeFormat: Format[MessageType] = Format[MessageType](Reads[MessageType] {
      case JsString(value) =>
